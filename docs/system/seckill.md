...
5. 如何解决超卖、少卖问题？  
&emsp; 秒杀的基本流程：在发布一个秒杀活动前，要将商品还有库存写入Redis缓存。当用户在前端页面进行抢购时，就通过操作Redis会对商品的库存进行扣减。在扣减完Redis中的库存数量后，再把扣减成功的消息发送到消息队列，之后就交给具体的业务系统进行处理，比如订单服务、支付服务等。在处理之后，再和MySql数据库进行交互，这样就完成了库存扣减的数据同步。    
