...
- [1. ~~缓存、Redis、Mq~~](#1-缓存redismq)
    - [1.1. 缓存](#11-缓存)
        - [1.1.1. ~~分布式缓存问题~~](#111-分布式缓存问题)
        - [1.1.2. 数据结构：bitMap、布隆、布谷鸟](#112-数据结构bitmap布隆布谷鸟)
            - [1.1.2.1. bitMap](#1121-bitmap)
            - [1.1.2.2. 布隆](#1122-布隆)
            - [1.1.2.3. 计数布隆过滤器](#1123-计数布隆过滤器)
            - [1.1.2.4. 布谷鸟](#1124-布谷鸟)
            - [1.1.2.5. Redis热点key](#1125-redis热点key)
        - [1.1.3. Redis](#113-redis)
            - [1.1.3.1. Redis数据类型](#1131-redis数据类型)
                - [1.1.3.1.1. Redis基本数据类型](#11311-redis基本数据类型)
                - [1.1.3.1.2. Redis扩展数据类型](#11312-redis扩展数据类型)
                - [1.1.3.1.3. redis使用：bigKey](#11313-redis使用bigkey)
                - [1.1.3.1.4. ~~Redis底层实现~~](#11314-redis底层实现)
                    - [1.1.3.1.4.1. 数据结构](#113141-数据结构)
                    - [1.1.3.1.4.2. SDS详解](#113142-sds详解)
                    - [1.1.3.1.4.3. Dictht](#113143-dictht)
                    - [1.1.3.1.4.4. 数据类型](#113144-数据类型)
            - [1.1.3.2. Redis原理](#1132-redis原理)
                - [1.1.3.2.1. Redis为什么那么快？](#11321-redis为什么那么快)
                - [1.1.3.2.2. Redis【虚拟内存】机制](#11322-redis虚拟内存机制)
                - [1.1.3.2.3. Redis事件/Reactor](#11323-redis事件reactor)
                - [1.1.3.2.4. Redis多线程模型](#11324-redis多线程模型)
                - [1.1.3.2.5. Redis协议](#11325-redis协议)
                - [1.1.3.2.6. Redis内存操作](#11326-redis内存操作)
                    - [1.1.3.2.6.1. Redis过期键删除](#113261-redis过期键删除)
                    - [1.1.3.2.6.2. Redis内存淘汰](#113262-redis内存淘汰)
                - [1.1.3.2.7. Redis持久化，磁盘操作](#11327-redis持久化磁盘操作)
                    - [1.1.3.2.7.1. ~~AOF重写阻塞~~](#113271-aof重写阻塞)
            - [1.1.3.3. Redis内置功能](#1133-redis内置功能)
                - [1.1.3.3.1. RedisPipeline/批处理](#11331-redispipeline批处理)
                - [1.1.3.3.2. Redis事务](#11332-redis事务)
                - [1.1.3.3.3. Redis和Lua](#11333-redis和lua)
                - [1.1.3.3.4. Redis实现消息队列](#11334-redis实现消息队列)
            - [1.1.3.4. Redis高可用](#1134-redis高可用)
                - [1.1.3.4.1. Redis高可用方案](#11341-redis高可用方案)
                - [1.1.3.4.2. Redis主从复制](#11342-redis主从复制)
                - [1.1.3.4.3. Redis读写分离](#11343-redis读写分离)
                - [1.1.3.4.4. Redis哨兵模式](#11344-redis哨兵模式)
                - [1.1.3.4.5. Redis集群模式](#11345-redis集群模式)
            - [1.1.3.5. Redis常见问题与优化](#1135-redis常见问题与优化)
        - [1.1.4. 分布式限流](#114-分布式限流)
        - [1.1.5. 服务降级](#115-服务降级)
    - [1.2. 分布式消息队列](#12-分布式消息队列)
        - [1.2.1. mq](#121-mq)
        - [1.2.2. Kafka](#122-kafka)
            - [1.2.2.1. kafka基本概念](#1221-kafka基本概念)
                - [1.2.2.1.1. kafka生产者](#12211-kafka生产者)
                - [1.2.2.1.2. 消息分区](#12212-消息分区)
                - [1.2.2.1.3. kafka消费者](#12213-kafka消费者)
                - [1.2.2.1.4. kafka服务端](#12214-kafka服务端)
            - [1.2.2.2. kafka特性](#1222-kafka特性)
                - [1.2.2.2.1. 【1】高性能(读写机制) ，Kafka为什么吞吐量大、速度快？](#12221-1高性能读写机制-kafka为什么吞吐量大速度快)
                    - [1.2.2.2.1.1. 内存](#122211-内存)
                    - [1.2.2.2.1.2. 持久化 / 磁盘IO - 顺序读写](#122212-持久化--磁盘io---顺序读写)
                    - [1.2.2.2.1.3. 网络IO优化](#122213-网络io优化)
                - [1.2.2.2.2. 【2】高可用与数据一致性(副本机制)](#12222-2高可用与数据一致性副本机制)
                - [1.2.2.2.3. ~~可靠性~~](#12223-可靠性)
                    - [1.2.2.2.3.1. 如何保证消息队列不丢失?](#122231-如何保证消息队列不丢失)
                    - [1.2.2.2.3.2. 分区保证消费顺序](#122232-分区保证消费顺序)
                    - [1.2.2.2.3.3. 消费语义介绍](#122233-消费语义介绍)
                    - [1.2.2.2.3.4. 幂等（重复消费）](#122234-幂等重复消费)
                    - [1.2.2.2.3.5. 事务](#122235-事务)
...
##### 1.2.2.2.1. 【1】高性能(读写机制) ，Kafka为什么吞吐量大、速度快？
