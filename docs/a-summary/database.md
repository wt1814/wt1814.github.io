...
- [1. 数据库](#1-数据库)
    - [1.1. SQL](#11-sql)
    - [1.2. 索引事物锁](#12-索引事物锁)
        - [1.2.1. 索引底层原理](#121-索引底层原理)
        - [1.2.2. ~~各种索引~~（还需要总结）](#122-各种索引还需要总结)
        - [1.2.3. MySql事务（还需要总结）](#123-mysql事务还需要总结)
        - [1.2.4. MVCC](#124-mvcc)
        - [1.2.5. MySql锁](#125-mysql锁)
        - [1.2.6. MySql死锁和锁表](#126-mysql死锁和锁表)
    - [1.3. ~~MySql优化~~](#13-mysql优化)
        - [1.3.1. SQL分析](#131-sql分析)
            - [1.3.1.1. Expain](#1311-expain)
        - [1.3.2. 索引优化](#132-索引优化)
        - [1.3.3. SQL语句优化](#133-sql语句优化)
        - [1.3.4. 碎片优化](#134-碎片优化)
        - [1.3.5. ~~Sql优化的一些案例~~](#135-sql优化的一些案例)
    - [1.4. 数据库分布式](#14-数据库分布式)
        - [1.4.1. 大数据量操作](#141-大数据量操作)
        - [1.4.2. MySql瓶颈](#142-mysql瓶颈)
        - [1.4.3. 数据库分布式](#143-数据库分布式)
        - [1.4.4. 主从复制](#144-主从复制)
            - [1.4.4.1. 主从复制原理](#1441-主从复制原理)
            - [1.4.4.2. 主从复制实现](#1442-主从复制实现)
            - [1.4.4.3. 主从复制问题](#1443-主从复制问题)
            - [1.4.4.4. 高可用实现](#1444-高可用实现)
            - [1.4.4.5. 读写分离实现](#1445-读写分离实现)
        - [1.4.5. 分区](#145-分区)
        - [1.4.6. ~~分库分表~~](#146-分库分表)
            - [1.4.6.1. 分库分表](#1461-分库分表)
            - [1.4.6.2. 分库分表查询](#1462-分库分表查询)
                - [1.4.6.2.1. 非partition key的查询 / 分库分表多维度查询](#14621-非partition-key的查询--分库分表多维度查询)
                    - [1.4.6.2.1.1. 跨分片的分组group by以及聚合count等函数](#146211-跨分片的分组group-by以及聚合count等函数)
                    - [1.4.6.2.1.2. 跨分片的排序分页](#146212-跨分片的排序分页)
                - [1.4.6.2.2. 跨节点Join的问题](#14622-跨节点join的问题)
                - [1.4.6.2.3. ~~**<font color = "blue">小结：分库分表分片键设计</font>**~~](#14623-font-color--blue小结分库分表分片键设计font)
        - [1.4.7. 数据迁移](#147-数据迁移)
        - [1.4.8. 大数据量](#148-大数据量)
        - [1.4.9. 数据库分布式实现](#149-数据库分布式实现)
            - [1.4.9.1. ShardingSphere](#1491-shardingsphere)
            - [1.4.9.2. sharding-jdbc事务](#1492-sharding-jdbc事务)
    - [1.5. MySql架构](#15-mysql架构)
        - [1.5.1. MySql运行流程](#151-mysql运行流程)
        - [1.5.2. Server层之binLog日志](#152-server层之binlog日志)
        - [1.5.3. 存储引擎层](#153-存储引擎层)
        - [1.5.4. InnoDB体系结构](#154-innodb体系结构)
            - [1.5.4.1. InnoDB内存结构-性能](#1541-innodb内存结构-性能)
                - [1.5.4.1.1. BufferPool](#15411-bufferpool)
                - [1.5.4.1.2. BufferPool落盘表空间](#15412-bufferpool落盘表空间)
                - [1.5.4.1.3. 写缓冲ChangeBuffer](#15413-写缓冲changebuffer)
                - [1.5.4.1.4. AdaptiveHashIndex](#15414-adaptivehashindex)
            - [1.5.4.2. InnoDB磁盘结构-可靠性](#1542-innodb磁盘结构-可靠性)
                - [1.5.4.2.1. undoLog](#15421-undolog)
                - [1.5.4.2.2. redoLog](#15422-redolog)
                - [1.5.4.2.3. DoubleWrite](#15423-doublewrite)
            - [1.5.4.3. ~~两阶段提交和崩溃恢复~~](#1543-两阶段提交和崩溃恢复)
...
#### 1.4.6.2. 分库分表查询
##### 1.4.6.2.1. 非partition key的查询 / 分库分表多维度查询  
...
###### 1.4.6.2.1.1. 跨分片的分组group by以及聚合count等函数  
...
##### 1.4.6.2.3. ~~**<font color = "blue">小结：分库分表分片键设计</font>**~~  
...
### 1.4.9. 数据库分布式实现  
#### 1.4.9.1. ShardingSphere
&emsp; sharding-jdbc与sharding-proxy比较：  
&emsp; 两者的主要区别在于ShardingJDBC更加注重数据库的分片和分布式事务，而Proxy更加注重数据库的负载均衡和故障转移。  

#### 1.4.9.2. sharding-jdbc事务  
1. 支持3种分布式事务  
...
4. 实现柔性事务  
&emsp; 这种模式，是由Seata作为事务协调者，来进行协调。  
