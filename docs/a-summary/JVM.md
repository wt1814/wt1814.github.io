...
- [1. JVM](#1-jvm)
    ...
    - [1.4. 运行时数据区/内存结构](#14-运行时数据区内存结构)
        - [1.4.1. JVM内存结构](#141-jvm内存结构)
            ...
            - [1.4.1.2. 类存储内存小结](#1412-类存储内存小结)
                - [1.4.1.2.1. 变量](#14121-变量)
                - [1.4.1.2.2. 静态方法和实例方法](#14122-静态方法和实例方法)
            - [1.4.1.3. 常量池详解](#1413-常量池详解)
        - [1.4.2. 内存(堆栈)中的对象](#142-内存堆栈中的对象)
            - [1.4.2.1. 创建对象](#1421-创建对象)
            - [1.4.2.2. 对象生命周期](#1422-对象生命周期)
            - [1.4.2.3. 对象大小](#1423-对象大小)
                - [1.4.2.3.1. 分析java对象的组成](#14231-分析java对象的组成)
                - [1.4.2.3.2. 通过代码计算对象的大小](#14232-通过代码计算对象的大小)
        - [1.4.3. 内存泄露](#143-内存泄露)
        - [1.4.4. JVM参数配置](#144-jvm参数配置)
    - [1.5. JVM执行](#15-jvm执行)
    - [1.6. GC](#16-gc)
        - [1.6.1. GC-回收位置/安全点](#161-gc-回收位置安全点)
        - [1.6.2. 回收算法与分代回收](#162-回收算法与分代回收)
        - [1.6.3. GC-回收对象](#163-gc-回收对象)
            - [1.6.3.1. 堆中对象的存活](#1631-堆中对象的存活)
            - [1.6.3.2. 方法区（类和常量）回收/类的卸载阶段](#1632-方法区类和常量回收类的卸载阶段)
            - [1.6.3.3. null与GC](#1633-null与gc)
        - [1.6.4. GC-垃圾回收器](#164-gc-垃圾回收器)
            - [1.6.4.1. 垃圾回收器](#1641-垃圾回收器)
            - [1.6.4.2. CMS回收器](#1642-cms回收器)
            - [1.6.4.3. G1回收器](#1643-g1回收器)
            - [1.6.4.4. 三色标记，并发标记阶段](#1644-三色标记并发标记阶段)
    - [1.7. JVM调优](#17-jvm调优)
