...
- [3. Hystrix原理](#3-hystrix原理)
    ...
    - [3.2. 熔断与降级](#32-熔断与降级)
        - [3.2.1. 熔断触发降级（断路器原理）](#321-熔断触发降级断路器原理)
        - [3.2.2. 请求超时触发降级](#322-请求超时触发降级)
        - [3.2.3. 资源隔离触发降级（依赖隔离）](#323-资源隔离触发降级依赖隔离)
...
2. 当熔断器开关打开时, 请求被禁止通过。  
3. 当熔断器开关处于打开状态, 经过一段时间后, 熔断器会自动进入半开状态, 这时熔断器只允许少部分请求通过。当该部分请求调用成功时, 熔断器恢复到关闭状态。若该请求失败, 熔断器继续保持打开状态, 接下来的请求被禁止通过。  
