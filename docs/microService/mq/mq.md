...
- [1. ~~mq~~](#1-mq)
    ...
    - [1.4. 消息队列使用事项](#14-消息队列使用事项)
        - [1.4.1. 保障可用性](#141-保障可用性)
        - [1.4.2. 丢失消息（可靠性传输）](#142-丢失消息可靠性传输)
        - [1.4.3. 重复消费](#143-重复消费)
        - [1.4.4. 顺序消费](#144-顺序消费)
        - [1.4.5. 消息积压(消费消息延迟)](#145-消息积压消费消息延迟)
            - [1.4.5.1. ~~大量消息在 mq 里积压了几个小时了还没解决~~](#1451-大量消息在-mq-里积压了几个小时了还没解决)
            - [1.4.5.2. mq 中的消息过期失效了](#1452-mq-中的消息过期失效了)
            - [1.4.5.3. mq 都快写满了](#1453-mq-都快写满了)
...
3. 消息队列使用事项
    1. 保障高可用  
    2. 消息队列丢失消息有3种情况：生产者丢失数据、mq客户端丢失数据、消费者丢失数据。  
    2. 重复消费  
        消费接口幂等性。  
    3. 顺序消费  
...
#### 1.4.5.1. ~~大量消息在 mq 里积压了几个小时了还没解决~~   
