...
- [1. 分片模式](#1-分片模式)
    ...
    - [1.3. Redis Cluster集群](#13-redis-cluster集群)
        ...
        - [1.3.2. 部署](#132-部署)
        - [1.3.3. 原理](#133-原理)
            - [1.3.3.1. Redis集群服务端](#1331-redis集群服务端)
                - [1.3.3.1.1. 数据分布](#13311-数据分布)
                    - [1.3.3.1.1.1. Redis数据分区](#133111-redis数据分区)
                    - [1.3.3.1.1.2. 集群功能限制](#133112-集群功能限制)
                - [1.3.3.1.2. 节点通信流程](#13312-节点通信流程)
                - [1.3.3.1.3. 集群伸缩](#13313-集群伸缩)
                    - [1.3.3.1.3.1. 伸缩原理](#133131-伸缩原理)
                    - [1.3.3.1.3.2. 扩容集群](#133132-扩容集群)
                    - [1.3.3.1.3.3. 收缩集群](#133133-收缩集群)
                - [1.3.3.1.4. 故障转移](#13314-故障转移)
                    - [1.3.3.1.4.1. 故障发现](#133141-故障发现)
                    - [1.3.3.1.4.2. 故障恢复](#133142-故障恢复)
            - [1.3.3.2. 服务端和客户端](#1332-服务端和客户端)
                - [1.3.3.2.1. 请求路由，客户端重定向](#13321-请求路由客户端重定向)
                    - [1.3.3.2.1.1. 请求重定向](#133211-请求重定向)
                    - [1.3.3.2.1.2. Smart客户端](#133212-smart客户端)
                    - [1.3.3.2.1.3. ASK重定向](#133213-ask重定向)
        - [1.3.4. 集群运维](#134-集群运维)
...
# 1. 分片模式  
&emsp; **<font color = "lime">Redis Cluster集群一句话概述：</font>**
...
###### 1.3.3.2.1.3. ASK重定向  
&emsp; <font color = "red">Redis集群支持在线迁移槽（slot）和数据来完成水平伸缩，当slot对应的数据从源节点到目标节点迁移过程中，客户端需要做到智能识别，保证键命令可正常执行。例如当一个slot数据从源节点迁移到目标节点时，期间可 能出现一部分数据在源节点，而另一部分在目标节点。</font>  
