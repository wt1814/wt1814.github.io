...
- [1. 分片模式](#1-分片模式)
    ...
    - [1.3. Redis Cluster集群](#13-redis-cluster集群)
        ...
        - [1.3.3. 原理](#133-原理)
            ...
            - [1.3.3.5. 请求路由，客户端重定向](#1335-请求路由客户端重定向)
                - [1.3.3.5.1. 请求重定向](#13351-请求重定向)
                - [1.3.3.5.2. Smart客户端](#13352-smart客户端)
                - [1.3.3.5.3. ASK重定向](#13353-ask重定向)
        - [1.3.4. 集群运维](#134-集群运维)
...
##### 1.3.3.5.3. ASK重定向  
&emsp; <font color = "red">Redis集群支持在线迁移槽（slot）和数据来完成水平伸缩，当slot对应 的数据从源节点到目标节点迁移过程中，客户端需要做到智能识别，保证键 命令可正常执行。例如当一个slot数据从源节点迁移到目标节点时，期间可 能出现一部分数据在源节点，而另一部分在目标节点。</font>  
