...
- [1. ~~主从复制模式~~](#1-主从复制模式)
    ...
    - [1.2. 复制流程](#12-复制流程)
        - [1.2.1. 复制流程概述](#121-复制流程概述)
        - [1.2.2. 数据同步阶段两种复制方式](#122-数据同步阶段两种复制方式)
            - [1.2.2.1. 全量复制](#1221-全量复制)
            - [1.2.2.2. 部分复制](#1222-部分复制)
                - [1.2.2.2.1. 基本概念](#12221-基本概念)
                    - [1.2.2.2.1.1. 复制偏移量](#122211-复制偏移量)
                    - [1.2.2.2.1.2. 复制积压缓冲区](#122212-复制积压缓冲区)
                    - [1.2.2.2.1.3. 运行 ID(runid)](#122213-运行-idrunid)
                - [1.2.2.2.2. 部分复制流程](#12222-部分复制流程)
            - [1.2.2.3. psync 命令的执行过程详解](#1223-psync-命令的执行过程详解)
    - [1.3. 主从复制应用与问题](#13-主从复制应用与问题)
        - [1.3.1. 传输延迟](#131-传输延迟)
        - [1.3.2. 规避全量复制](#132-规避全量复制)
            - [1.3.2.1. 第一次复制](#1321-第一次复制)
            - [1.3.2.2. 节点运行ID不匹配](#1322-节点运行id不匹配)
            - [1.3.2.3. 复制偏移量 offset 不在复制积压缓冲区中](#1323-复制偏移量-offset-不在复制积压缓冲区中)
        - [1.3.3. 规避复制风暴](#133-规避复制风暴)
            - [1.3.3.1. 单一主节点](#1331-单一主节点)
            - [1.3.3.2. 单一机器](#1332-单一机器)
    - [1.4. 主从复制使用](#14-主从复制使用)
...
### 1.3.1. 传输延迟    
&emsp; 主从节点一般部署在不同机器上，复制时的网络延迟就成为需要考虑的 问题，Redis为我们提供了repl-disable-tcp-nodelay参数用于控制是否关闭 TCP_NODELAY，默认关闭，说明如下：  
...
#### 1.3.2.3. 复制偏移量 offset 不在复制积压缓冲区中  
