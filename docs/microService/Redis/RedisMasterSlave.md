...
&emsp; 二、数据同步阶段：5. 同步数据集。  
&emsp; 三、命令传播阶段：6. 命令持续复制。  

&emsp; 数据同步阶段两种复制方式：全量复制和部分复制。  
...
* 复制偏移量 offset 不在复制积压缓冲区中。需要根据中断时长来调整复制积压缓冲区的大小
...
&emsp; 关于连接失败，可以在从节点执行 info replication查看 master_link_down_since_seconds指标，它会记录与主节点连接失败的系统时间。从节点连接主节点失败时也会每秒打印如下日志，方便发现问题：  
