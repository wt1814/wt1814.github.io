...
- [1. ~~Saga~~](#1-saga)
    ...
    - [1.2. Saga的使用条件](#12-saga的使用条件)
    - [1.3. Saga和TCC](#13-saga和tcc)
    - [1.4. Saga流程](#14-saga流程)
...
&emsp; <font color = "blue">Saga模型将一个分布式事务拆分为多个本地事务，每个本地事务都有相应的执行模块和补偿模块(对应TCC中的Confirm和Cancel)，当Saga事务中任意一个本地事务出错时，可以通过调用相关的补偿方法恢复之前的事务，达到事务最终一致性。</font>  
