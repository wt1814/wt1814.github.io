...
- [3. 缓存击穿](#3-缓存击穿)
    - [3.1. 问题](#31-问题)
    - [3.2. 解决方案](#32-解决方案)
        - [3.2.1. 互斥锁方案详解](#321-互斥锁方案详解)
        - [3.2.2. 双缓存](#322-双缓存)
- [4. 保证缓存与数据库的双写一致性](#4-保证缓存与数据库的双写一致性)
    - [4.1. 问题](#41-问题)
    - [4.2. 解决方案](#42-解决方案)
        - [4.2.1. 使用辅助队列](#421-使用辅助队列)
...
2. 设置布隆过滤器：  
&emsp; [布隆过滤器介绍](../../java/function/3.otherStructure.md)  
...
1. 多个key同时失效：  
    ...
    * **<font color = "red">使用互斥锁或队列：在缓存失效后，通过互斥锁或者队列，控制读数据库和写缓存的线程数量；不过这样会导致系统的吞吐量下降。</font>**  
    * **<font color = "red">双缓存：设置一级缓存和二级缓存，一级缓存过期时间短，二级缓存过期时间长或者不过期，一级缓存失效后访问二级缓存，同时刷新一级缓存。</font>**  
...
* 使用互斥锁。加锁有多种方式。<!-- **分布式环境中要使用分布式锁，单机的话用普通的锁（synchronized、Lock）。**  --> 
