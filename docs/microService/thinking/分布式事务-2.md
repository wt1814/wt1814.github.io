...
- [1. 分布式事务](#1-分布式事务)
    - [1.1. 消息驱动模式（Message Driven）](#11-消息驱动模式message-driven)
        - [1.1.1. 本地消息表（异步确保）-强一致性](#111-本地消息表异步确保-强一致性)
        ...
        - [1.1.3. 最大努力通知-最终一致性](#113-最大努力通知-最终一致性)
            ...
            - [1.1.3.2. 特点](#1132-特点)
    - [1.2. 事件溯源模式(Event Sourcing)，Saga事务模型-最终一致性](#12-事件溯源模式event-sourcingsaga事务模型-最终一致性)
        - [1.2.1. Saga组成](#121-saga组成)
        - [1.2.2. Saga的使用条件](#122-saga的使用条件)
        - [1.2.3. Saga相关实现](#123-saga相关实现)
        - [1.2.4. Saga协调](#124-saga协调)
            - [1.2.4.1. 编排（Choreography）](#1241-编排choreography)
                - [1.2.4.1.1. 实现流程](#12411-实现流程)
                - [1.2.4.1.2. 特点](#12412-特点)
            - [1.2.4.2. 控制（Orchestration）](#1242-控制orchestration)
                - [1.2.4.2.1. 实现流程](#12421-实现流程)
                    - [1.2.4.2.1.1. 使用状态机建模SAGA ORCHESTRATORS](#124211-使用状态机建模saga-orchestrators)
                    - [1.2.4.2.1.2. SAGA控制和事务消息](#124212-saga控制和事务消息)
                - [1.2.4.2.2. 特点](#12422-特点)

<!-- /TOC -->

# 1. 分布式事务
## 1.1. 消息驱动模式（Message Driven） 
### 1.1.1. 本地消息表（异步确保）-强一致性  
...
## 1.2. 事件溯源模式(Event Sourcing)，Saga事务模型-最终一致性  
<!--
...
###### 1.2.4.2.1.1. 使用状态机建模SAGA ORCHESTRATORS  
