...
- [分布式事务](#分布式事务)
    ...
    - [2. 基于DTP的XA实现](#2-基于dtp的xa实现)
        - [2.1. 一致性协议-XA两阶段提交](#21-一致性协议-xa两阶段提交)
            - [2.1.1. 实现流程](#211-实现流程)
            - [2.1.2. 特点](#212-特点)
            - [2.1.3. XA二阶段问题](#213-xa二阶段问题)
        - [2.2. 一致性协议-XA三阶段提交](#22-一致性协议-xa三阶段提交)
            - [2.2.1. 实现流程](#221-实现流程)
            - [2.2.2. 特点](#222-特点)
...
* 优点：相比二阶段提交，<font color = "red">三阶段提交降低了阻塞范围</font>，在等待超时后协调者或参与者会中断事务；<font color = "red">避免了协调者单点问题</font>，阶段 3 中协调者出现问题时，参与者会继续提交事务。  
