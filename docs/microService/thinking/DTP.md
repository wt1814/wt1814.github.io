...
&emsp; **XA二阶段问题：** 1.单点故障问题；2.同步阻塞问题；3.丢失消息导致的数据不一致问题；4.二阶段无法解决的问题：协调者和参与者同时宕机。  

&emsp; **3PC：**  
&emsp; **<font color = "clime">3PC的引入是为了解决2PC的同步阻塞，解决协调者和参与者同时挂掉的问题，减少了数据不一致的情况。</font>**  
...
&emsp; **<font color = "red">同时在协调者和参与者中都引入超时机制。一旦事物参与者迟迟没有接到协调者的commit请求，会自动进行本地commit。这样有效解决了协调者单点故障的问题。</font>**  
&emsp; <font color = "red">三阶段提交降低了阻塞范围。</font>  
...
3. 丢失消息导致的数据不一致问题：  
&emsp; 在二阶段提交的阶段二中，当协调者向参与者发送commit请求之后，发生了局部网络异常或者在发送commit请求过程中协调者发生了故障，这会导致只有一部分参与者接受到了commit请求。而这部分参与者接到commit请求之后就会执行commit操作。但是其他部分未接到commit请求的机器则无法执行事务提交。于是整个分布式系统便出现了数据部一致性的现象。  
...
降低了锁定资源的概率和时长
a. can commit资源不锁定。
b. tm 协调者和 rm资源拥有者的 处理机制。
tm：未收到反馈，给rm发 中断事务的命令；
rm：在3阶段，没收到tm的命令，默认提交。
-->
&emsp; **<font color = "clime">3PC的引入是为了解决2PC的同步阻塞，解决协调者和参与者同时挂掉的问题，减少了数据不一致的情况。</font>**  
...
1. 引入超时机制。 **<font color = "red">同时在协调者和参与者中都引入超时机制。一旦事物参与者迟迟没有接到协调者的commit请求，会自动进行本地commit。这样有效解决了协调者单点故障的问题。</font>** 但是性能问题和不一致的问题仍然没有根本解决。  
