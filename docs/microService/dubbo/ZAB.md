...
- [1. 服务端的ZAB协议](#1-服务端的zab协议)
    ...
    - [1.3. ZAB协议](#13-zab协议)
        - [1.3.1. 服务端主从节点的数据同步，广播模式-数据读写流程(读写分离)](#131-服务端主从节点的数据同步广播模式-数据读写流程读写分离)
        - [1.3.2. 选举流程，恢复模式](#132-选举流程恢复模式)
            - [1.3.2.1. 选举流程中几个重要参数](#1321-选举流程中几个重要参数)
            - [1.3.2.2. 服务器启动时的leader选举](#1322-服务器启动时的leader选举)
            - [1.3.2.3. 运行过程中的leader选举](#1323-运行过程中的leader选举)
...
2. ZAB协议：
    ...
    2. **<font color = "clime">`消息广播（数据读写流程）：`</font>**  
...
### 1.3.1. 服务端主从节点的数据同步，广播模式-数据读写流程(读写分离) 
