...
&emsp; <font color="red">不同的是缓存穿透是数据库和缓存都不存在相关数据；而缓存击穿和缓存雪崩是缓存和数据库都存在相应数据，只是缓存失效了而已。</font>  

&emsp; 简单解决方案：  
1. 存穿透，空值缓存；  
2. 缓存击穿和缓存雪崩，可以设置缓存时间为随机值或永久不过期；  
3. 缓存与数据库的双写一致性，可以使用延迟双删策略。  
...
2. 设置布隆过滤器：  
&emsp; [布隆过滤器介绍](/docs/java/function/otherStructure.md)  
...
### 1.2.2. 解决方案  
1. **<font color = "lime">多个key同时失效：</font>**  
    * **<font color = "lime">不设置缓存过期时间。</font>**  
    * **<font color = "lime">设置随机过期时间，这样就会大幅度的减少缓存在同一时间过期。</font>**  
