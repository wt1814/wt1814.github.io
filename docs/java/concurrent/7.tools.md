...
- [1. Tools，工具类](#1-tools工具类)
    - [1.1. CountDownLatch，线程计数器](#11-countdownlatch线程计数器)
        ...
        - [1.1.4. await()](#114-await)
        - [1.1.5. countDown()](#115-countdown)
    - [1.2. 总结](#12-总结)
    - [1.3. CyclicBarrier，回环栅栏](#13-cyclicbarrier回环栅栏)
        - [1.3.1. API](#131-api)
        - [1.3.2. 实际使用场景](#132-实际使用场景)
        - [1.3.3. 使用示例](#133-使用示例)
        - [1.3.4. CycliBarriar和CountdownLatch有什么区别？](#134-cyclibarriar和countdownlatch有什么区别)
        - [1.3.5. ※※※CycliBarriar和CountdownLatch具体使用](#135-※※※cyclibarriar和countdownlatch具体使用)
    - [1.4. Semaphore，信号量-控制同时访问的线程个数](#14-semaphore信号量-控制同时访问的线程个数)
        - [1.4.1. API](#141-api)
        - [1.4.2. 使用示例](#142-使用示例)
    - [1.5. Exchanger，交换器](#15-exchanger交换器)
...
&emsp; CountDownLatch是通过一个计数器来实现的，当我们在new 一个CountDownLatch对象的时候需要带入该计数器值，该值就表示了线程的数量。每当一个线程完成自己的任务后，计数器的值就会减1。当计数器的值变为0时，就表示所有的线程均已经完成了任务，然后就可以恢复等待的线程继续执行了。  

#### 1.1.3.1. 类结构  
&emsp; CountDownLatch只有一个属性Sync，Sync是继承了AQS的内部类。  
&emsp; 创建CountDownLatch时传入一个count值，count值被赋值给AQS.state。  
&emsp; CountDownLatch是通过AQS共享锁实现的，AQS这篇文章中详细讲解了AQS独占锁的原理，AQS共享锁和独占锁原理只有很细微的区别，这里大致介绍下：  
...
<font color = "lime">创建CountDownLatch时设置AQS.state=count，可以理解成锁被重入了count次。await()方法获取锁时锁被占用了，只能阻塞。</font>  
...
### 1.1.5. countDown()  
&emsp; countDown()方法是将count-1，如果发现count=0了，就唤醒阻塞的线程。  
...
&emsp; CountDownLatch是由AQS实现的，创建CountDownLatch时设置计数器count其实就是设置AQS.state=count，也就是重入次数。await()方法调用获取锁的方法，由于AQS.state=count表示锁被占用且重入次数为count，所以获取不到锁线程被阻塞并进入AQS队列。countDown()方法调用释放锁的方法，每释放一次AQS.state减1，当AQS.state变为0时表示处于无锁状态了，就依次唤醒AQS队列中阻塞的线程来获取锁，继续执行逻辑代码。  


## 1.3. CyclicBarrier，回环栅栏  
<!--
CyclicBarrier 使用详解
...
### 1.3.4. CycliBarriar和CountdownLatch有什么区别？  
...
### 1.3.5. ※※※CycliBarriar和CountdownLatch具体使用  
&emsp; [多线程处理大数据量](/docs/java/concurrent/bigData.md)    

## 1.4. Semaphore，信号量-控制同时访问的线程个数  
