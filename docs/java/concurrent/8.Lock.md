...
- [1. java.util.concurrent.locks包](#1-javautilconcurrentlocks包)
    - [1.1. ReentrantLock，重入锁](#11-reentrantlock重入锁)
        - [ReentrantLock解析](#reentrantlock解析)
            - [ReentrantLock类层次结构](#reentrantlock类层次结构)
            - [非公平锁的lock方法](#非公平锁的lock方法)
...
2、当我们调用ReentrantLock的lock方法的时候，实际上是调用了NonfairSync的lock方法，这个方法先用CAS操作，去尝试抢占该锁。如果成功，就把当前线程设置在这个锁上，表示抢占成功。如果失败，则调用acquire模板方法，等待抢占。
