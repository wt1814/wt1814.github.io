...
- [1. AQS](#1-aqs)
    - [1.1. 属性](#11-属性)
        ...
        - [1.1.2. 同步队列](#112-同步队列)
            - [1.1.2.1. 队列描述](#1121-队列描述)
            - [1.1.2.2. 入列以及出列动作](#1122-入列以及出列动作)
                - [1.1.2.2.1. 入列](#11221-入列)
                - [1.1.2.2.2. 出列](#11222-出列)
        - [1.1.3. 等待队列](#113-等待队列)
    - [1.2. 成员方法](#12-成员方法)
        - [1.2.1. 独占模式](#121-独占模式)
            - [1.2.1.1. 获取同步状态--acquire()](#1211-获取同步状态--acquire)
            - [1.2.1.2. 释放同步状态--release()](#1212-释放同步状态--release)
        - [1.2.2. 共享模式](#122-共享模式)
            - [1.2.2.1. 获取同步状态--acquireShared-1](#1221-获取同步状态--acquireshared-1)
            - [1.2.2.2. 释放同步状态--releaseShared](#1222-释放同步状态--releaseshared)
        - [1.2.3. AQS的模板方法设计模式，自定义同步器](#123-aqs的模板方法设计模式自定义同步器)
