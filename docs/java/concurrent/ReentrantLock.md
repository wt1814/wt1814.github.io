...
- [1. ReentrantLock解析](#1-reentrantlock解析)
    - [1.1. ReentrantLock类层次结构](#11-reentrantlock类层次结构)
    - [1.2. 构造函数](#12-构造函数)
    - [1.3. 成员方法](#13-成员方法)
    - [1.4. 非公平锁NonfairSync解析](#14-非公平锁nonfairsync解析)
        - [1.4.1. 获取锁lock()](#141-获取锁lock)
            - [1.4.1.1. 时序图](#1411-时序图)
            - [1.4.1.2. ~~源码解析~~](#1412-源码解析)
        - [1.4.2. 释放锁unlock()](#142-释放锁unlock)
    - [1.5. 公平锁FairSync](#15-公平锁fairsync)

<!-- /TOC -->

&emsp; **<font color = "red">总结：</font>**  
1. **<font color = "red">lock()方法描述：★★★NonfairSync --- acquire模板方法</font>**  
...
# 1. ReentrantLock解析  
## 1.1. ReentrantLock类层次结构  
![image](http://182.92.69.8:8081/img/java/concurrent/concurrent-26.png)  
...
![image](http://182.92.69.8:8081/img/java/concurrent/concurrent-27.png)   
...
&emsp; `**<font color = "red">`“非公平”体现在，如果占用锁的线程刚释放锁，state置为0，而排队等待锁的线程还未唤醒时，新来的线程就直接抢占了该锁，那么就“插队”了。`   
...
static final class NonfairSync extends Sync {
    ...
    final void lock() {
        ...
        if (compareAndSetState(0, 1)) //todo 插队操作
...
2. 第二步，线程B和线程C执行tryAcquire失败，入等待队列`addWaiter(Node mode)`。如果线程A拿着锁死死不放，那么B和C就会被挂起。  
    ...
    */
    ...
    ![image](http://182.92.69.8:8081/img/java/concurrent/multi-73.png)  
3. 第三步，B和C相继执行acquireQueued(final Node node, int arg)。这个方法让已经入队的线程尝试获取锁，若失败则会被挂起。  
    ...
    */
    ...
    &emsp; 最终队列可能会如下图所示：  
    ![image](http://182.92.69.8:8081/img/java/concurrent/multi-74.png) 
...
### 1.4.2. 释放锁unlock()
&emsp; unlock方法的时序图  
![image](http://182.92.69.8:8081/img/java/concurrent/multi-72.png)  
