    ...
    - [1.2. AQS，抽象队列同步器，基础组件](#12-aqs抽象队列同步器基础组件)
        ...
        - [1.2.2. 成员方法](#122-成员方法)
            ...
            - [1.2.2.2. 共享模式](#1222-共享模式)
                - [1.2.2.2.1. 获取同步状态--acquireShared](#12221-获取同步状态--acquireshared)
                - [1.2.2.2.2. 释放同步状态--releaseShared](#12222-释放同步状态--releaseshared)
...
&emsp; 函数流程如下：  
1. 调用使用者重写的tryAcquire方法，tryAcquire()尝试直接去获取资源，如果成功则直接返回（这里体现了非公平锁，每个线程获取锁时会尝试直接抢占加塞一次，而CLH队列中可能还有别的线程在等待）；  
