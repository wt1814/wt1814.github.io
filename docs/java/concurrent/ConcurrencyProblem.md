...
- [1. 并发安全问题产生原因](#1-并发安全问题产生原因)
    - [1.1. 并发安全问题及含义](#11-并发安全问题及含义)
        - [1.1.1. 线程切换带来的原子性问题](#111-线程切换带来的原子性问题)
        - [1.1.2. 缓存导致了可见性问题](#112-缓存导致了可见性问题)
        - [1.1.3. 编译优化带来的有序性问题](#113-编译优化带来的有序性问题)
            - [1.1.3.1. 重排序](#1131-重排序)
            - [1.1.3.2. 重排序分类](#1132-重排序分类)
            - [1.1.3.3. 重排序规则](#1133-重排序规则)
                - [1.1.3.3.1. 重排序遵守数据依赖性](#11331-重排序遵守数据依赖性)
                - [1.1.3.3.2. 重排序遵守as-if-serial语义](#11332-重排序遵守as-if-serial语义)
            - [1.1.3.4. 重排序对多线程的影响](#1134-重排序对多线程的影响)
