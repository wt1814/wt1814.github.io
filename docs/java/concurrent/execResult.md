...
## 1.5. 异步回调
&emsp; 上面列举的获取线程执行结果的方法都存在一个共性的问题，就是在等待结果的返回过程中，主线程或者消费者线程都是需要阻塞或轮询等待的，但在一些应用场景下我们是希望线程执行的过程中，程序该干嘛干嘛，继续向下执行，等到结果返回了再通过回调来通知，这就是异步回调的必要性。实现异步回调思路我这里列举两种，一种是多线程与回调，第二种JDK1.8中新加入了一个实现类CompletableFuture，通过这两种都能够实现异步获取线程执行结果的目标  
