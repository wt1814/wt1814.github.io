...
1. ReentrantReadWriteLock  
    ...
    2. **<font color = "red">ReentrantReadWriteLock缺点：`读写锁互斥`，只有当前没有线程持有读锁或者写锁时，才能获取到写锁，</font><font color = "clime">这`可能会导致写线程发生饥饿现象`，</font><font color = "red">即`读线程太多导致写线程迟迟竞争不到锁`而一直处于等待状态。StampedLock()可以解决这个问题。</font>**  
2. StampedLock  
...
&emsp; **StampedLock通过乐观读锁Optimistic reading解决ReentrantReadWriteLock的写锁解饿问题，即一个线程获取乐观读锁之后，不会阻塞其他线程获取写锁。**  
1. **乐观读锁如何保证数据一致性呢？** 使用版本号。  
...
&emsp; 通过版本号控制。乐观读不能保证读取到的数据是最新的，所以将数据读取到局部变量的时候需要通过 lock.validate(stamp) 校验是否被写线程修改过，若是修改过则需要上悲观读锁，再重新读取数据到局部变量。 **<font color = "blue">即乐观读失败后，再次使用悲观读锁。</font>**    
