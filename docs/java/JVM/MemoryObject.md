...
1. **<font color = "clime">对象创建过程：1. 检测类`是否被加载`；2. 为对象`分配内存`；3. 将分配内存空间的对象`初始化零值`；4. 对对象进行`其他设置`；5.执行`init方法`。</font>**   
...
&emsp; 当剩余的空间小于最大浪费空间，那该TLAB属于的线程再重新向Eden区申请一个TLAB空间。进行对象创建，还是空间不够，那这个对象太大了，去Eden区直接创建吧！  
