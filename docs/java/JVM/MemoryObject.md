...
2. 为对象分配内存：
    * 分配内存两种方式：指针碰撞(内存空间绝对规整)；空闲列表（内存空间是不连续的）。标记-整理或复制 ---> 空间规整 ---> 指针碰撞； 标记-清除 ---> 空间不规整 ---> 空闲列表。       
    * 线程安全问题：采用CAS； **<font color = "clime">本地线程分配缓冲(TLAB)。</font>**  
    * 内存分配全流程：逃逸分析 ---> 没有逃逸，尝试栈上分配 ---> 是否满足直接进入老年代的条件 ---> 尝试TLAB分配 ---> Eden分配。  
3. 堆内存分配策略：  
&emsp; 空间分配担保：**<font color = "clime">JVM在发生Minor GC之前，虚拟机会检查老年代最大可用的连续空间是否大于新生代所有对象的总空间，</font>**   
...
&emsp; **<font color = "clime">小结：标记-整理或复制 ---> 空间规整 ---> 指针碰撞； 标记-清除 ---> 空间不规整 ---> 空闲列表。</font>**  
