...
- [1. G1](#1-g1)
    ...
    - [1.2. ~~基本概念~~](#12-基本概念)
        ...
        - [1.2.3. 卡片 Card](#123-卡片-card)
        - [1.2.4. RememberSets，记忆集](#124-remembersets记忆集)
        - [1.2.5. CollectionSets](#125-collectionsets)
        - [~~开始时快照技术SATB~~](#开始时快照技术satb)
    - [1.3. 回收流程](#13-回收流程)
...
1. G1，垃圾优先，可预测停顿模型。  
2. G1的收集过程可能有4个阶段：新生代GC、并发标记周期、混合回收、如果需要可能会进行Full GC。   
3. 混合回收MixGC步骤分2步：全局并发标记（global concurrent marking）、拷贝存活对象（evacuation）。
    1. 全局并发标记  
        1. 初始标记
        2. 根区域扫描
        3. 并发标记
...
&emsp; 由于Humongous对象会独占整个内存分段，如果Humongous对象变为垃圾，则内存分段百分百为垃圾，所以会在第一时间被回收掉。  
7. 恢复应用程序线程的执行。  
...
4. 最终标记（Remark，STW）  
&emsp; 重新标记(Remark)是最后一个标记阶段。在该阶段中，G1需要一个暂停的时间，去处理剩下的SATB日志缓冲区和所有更新，找出所有未被访问的存活对象，同时安全完成存活数据计算。这个阶段也是并行执行的，通过参数-XX:ParallelGCThread可设置GC暂停时可用的GC线程数。同时，引用处理也是重新标记阶段的一部分，所有重度使用引用对象(弱引用、软引用、虚引用、最终引用)的应用都会在引用处理上产生开销。
