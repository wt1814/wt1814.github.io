...
- [1. ~~G1~~](#1-g1)
    ...
    - [1.2. ~~基本概念~~](#12-基本概念)
        ...
        - [1.2.4. RememberSets(记忆集)，弱分代引用假说](#124-remembersets记忆集弱分代引用假说)
...
&emsp; 步骤分2步：全局并发标记（global concurrent marking）、拷贝存活对象（evacuation）。  
    1. 全局并发标记  
        ...
        2. 根区域扫描
        3. 并发标记
        4. 最终标记： **<font color = "blue">去处理剩下的SATB日志缓冲区和所有更新，找出所有未被访问的存活对象，同时安全完成存活数据计算。</font>**   
        5. 清除垃圾
...
### 1.2.4. RememberSets(记忆集)，弱分代引用假说
<!-- 
