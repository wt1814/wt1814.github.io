...
![image](http://182.92.69.8:8081/img/java/JVM/JVM-22.png)  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-18.png)  
&emsp; **<font color = "clime">Java中引入四种引用的目的是让程序自己决定对象的生命周期，JVM是通过垃圾回收器对这四种引用做不同的处理，来实现对象生命周期的改变。</font>**  

<!--
四种引用
https://juejin.cn/post/6844904040346681352#heading-6
...
![image](http://182.92.69.8:8081/img/java/JVM/JVM-19.png)  
&emsp; 运行结果：  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-20.png)  
&emsp; 打开被注释掉的new byte[1024*100]语句，这条语句请求一块大的堆空间，使堆内存使用紧张。并显式的再调用一次GC，结果如下：  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-21.png)说明在系统内存紧张的情况下，软引用被回收。  
...
![image](http://182.92.69.8:8081/img/java/JVM/JVM-23.png)  
&emsp; 运行结果：  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-24.png)  
...
![image](http://182.92.69.8:8081/img/java/JVM/JVM-25.png)  
&emsp; 运行结果：  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-26.png)  
&emsp; 对虚引用的get()操作，总是返回null，因为sf.get()方法的实现如下：  
![image](http://182.92.69.8:8081/img/java/JVM/JVM-27.png)  
...
4. 类的卸载
	...
	2. 类的卸载：  
		...
		&emsp; 注：由java虚拟机自带的三种类加载加载的类在虚拟机的整个生命周期中是不会被卸载的，由用户自定义的类加载器所加载的类才可以被卸载。    
		![image](http://182.92.69.8:8081/img/java/JVM/JVM-161.png)  
