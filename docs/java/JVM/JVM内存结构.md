...
- [1. JVM内存结构/运行时数据区](#1-jvm内存结构运行时数据区)
    ...
    - [1.3. 本地方法栈](#13-本地方法栈)
    - [1.4. 堆(heap)](#14-堆heap)
        - [1.4.1. 堆是分配对象存储的唯一选择吗？（逃逸分析）](#141-堆是分配对象存储的唯一选择吗逃逸分析)
            - [1.4.1.1. 逃逸分析的概念](#1411-逃逸分析的概念)
            - [1.4.1.2. 对象逃逸示例](#1412-对象逃逸示例)
            - [1.4.1.3. 逃逸分析原则](#1413-逃逸分析原则)
            - [1.4.1.4. 逃逸分析优化](#1414-逃逸分析优化)
                - [1.4.1.4.1. 对象可能分配在栈上](#14141-对象可能分配在栈上)
                - [1.4.1.4.2. 分离对象或标量替换](#14142-分离对象或标量替换)
                - [1.4.1.4.3. 同步锁消除](#14143-同步锁消除)
        - [1.4.2. Java堆内存配置项](#142-java堆内存配置项)
        - [1.4.3. 堆和栈的区别是什么？](#143-堆和栈的区别是什么)
        - [1.4.4. 堆和非堆内存](#144-堆和非堆内存)
