...
- [1. List](#1-list)
    - [1.1. ArrayList](#11-arraylist)
        - [1.1.1. 如何去掉list集合中重复的元素?](#111-如何去掉list集合中重复的元素)
        - [1.1.2. ArrayList 类定义](#112-arraylist-类定义)
        - [1.1.3. 字段属性](#113-字段属性)
        - [1.1.4. 构造函数](#114-构造函数)
        - [1.1.5. 成员方法（增、删、改、查）](#115-成员方法增删改查)
            - [1.1.5.1. 增加，add(E e)](#1151-增加adde-e)
            - [1.1.5.2. 删除，remove(int index)、remove(Object o)](#1152-删除removeint-indexremoveobject-o)
                - [1.1.5.2.1. 根据索引删除元素](#11521-根据索引删除元素)
                - [1.1.5.2.2. 直接删除指定元素](#11522-直接删除指定元素)
- [2. Set](#2-set)
...
3. java 8中去重操作  
&emsp; 利用jdk1.8中提供的Stream.distinct()列表去重，Stream.distinct()使用hashCode()和equals()方法来获取不同的元素，因此使用这种写法，对象需要重写hashCode()和equals()方法！  
4. HashSet去重操作。  
...
#### 1.1.5.2. 删除，remove(int index)、remove(Object o)  
##### 1.1.5.2.1. 根据索引删除元素  
...
```
&emsp; 第一个定义一个 HashMap，作为实现 HashSet 的数据结构；第二个 PRESENT 对象。 **<font color = "lime">存储在 HashSet 中的数据 作为 Map 的 key，而 Map 的value 统一为 PRESENT。</font>**
