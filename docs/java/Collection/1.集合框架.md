...
- [3. ConcurrentModificationException与fail-fast](#3-concurrentmodificationexception与fail-fast)
    - [3.1. ConcurrentModificationException异常-1](#31-concurrentmodificationexception异常-1)
        - [3.1.1. 单线程ArrayList修改元素时，防止抛出ConcurrentModificationException](#311-单线程arraylist修改元素时防止抛出concurrentmodificationexception)
...
5. **<font color = "lime">HashMap与HashTable的区别？</font>**  
    ...
    1. 两者父类不同：HashMap 和 Hashtable 都实现了 Map 接口，但 Hashtable 继承于 Dictionary 类，而 HashMap 是继承于 AbstractMap  
...
### 3.1.1. 单线程ArrayList修改元素时，防止抛出ConcurrentModificationException  
