...
- [1. 日志收集](#1-日志收集)
    - [1.1. ELK](#11-elk)
        - [1.1.1. ELK日志架构的演进-1](#111-elk日志架构的演进-1)
            - [1.1.1.1. Demo版](#1111-demo版)
            - [1.1.1.2. 初级版](#1112-初级版)
            - [1.1.1.3. 中级版](#1113-中级版)
            - [1.1.1.4. 高级版](#1114-高级版)
        - [1.1.2. ELK原理](#112-elk原理)
            - [1.1.2.1. Filebeat工作原理](#1121-filebeat工作原理)
            - [1.1.2.2. Logstash工作原理](#1122-logstash工作原理)
        - [1.1.3. ELK的搭建](#113-elk的搭建)
        - [1.1.4. ELK使用](#114-elk使用)
            - [配置lockback](#配置lockback)
    - [1.2. EFK，收集容器日志](#12-efk收集容器日志)
...
## 1.1. ELK
&emsp; ELK指的是Elasticsearch＋Logstash＋Kibana。  
...
&emsp; <font color = "red">ELK+Filebeat的流程：Filebeat->Logstash->（Elasticsearch<->Kibana）。由程序产生出日志，由Filebeat进行处理，将日志数据输出到Logstash中，Logstash再将数据输出到Elasticsearch中，Elasticsearch再与Kibana相结合展示给用户。</font>
...
&emsp; elasticsearch中文社区：  

    https://elasticsearch.cn/

#### 1.1.2.1. Filebeat工作原理  
...
&emsp; 一些常用的过滤器为：  
&emsp; grok：解析任意文本数据，Grok 是 Logstash 最重要的插件。它的主要作用就是将文本格式的字符串，转换成为具体的结构化的数据，配合正则表达式使用。内置120多个解析语法。  

    官方提供的grok表达式：https://github.com/logstash-plugins/logstash-patterns-core/tree/master/patterns
    grok在线调试：https://grokdebug.herokuapp.com/

&emsp; mutate：对字段进行转换。例如对字段进行删除、替换、修改、重命名等。  
&emsp; drop：丢弃一部分events不进行处理。  
&emsp; clone：拷贝 event，这个过程中也可以添加或移除字段。  
&emsp; geoip：添加地理信息(为前台kibana图形化展示使用)  
...
&emsp; efk就是目前最受欢迎的日志管理系统。kubernetes可以实现efk的快速部署和使用，通过statefulset控制器部署elasticsearch组件，用来存储日志数据，还可通过volumenclaimtemplate动态生成pv实现es数据的持久化。通过deployment部署kibana组件，实现日志的可视化管理。通过daemonset控制器部署fluentd组件，来收集各节点和k8s集群的日志。  
