...
- [4. EXPLAIN、PROCEDURE ANALYSE(分析)](#4-explainprocedure-analyse分析)
    - [4.1. explain与explain extended](#41-explain与explain-extended)
        - [explain extended](#explain-extended)
    - [4.2. show warnings](#42-show-warnings)
    - [4.3. ~~proceduer analyse()取得建议~~](#43-proceduer-analyse取得建议)
    - [4.4. profiling](#44-profiling)
        - [4.4.1. 查看 profile 开启情况](#441-查看-profile-开启情况)
        - [4.4.2. 启用 profile](#442-启用-profile)
        - [4.4.3. 查看执行的 SQL 列表](#443-查看执行的-sql-列表)
        - [4.4.4. 查询指定ID的执行详细信息](#444-查询指定id的执行详细信息)
        - [4.4.5. 获取CPU、Block IO等信息](#445-获取cpublock-io等信息)
- [5. Sql语句优化](#5-sql语句优化)
...
- [6. 数据库表结构设计](#6-数据库表结构设计)
    - [6.1. 选择合适的数据类型](#61-选择合适的数据类型)
    - [6.2. 表的拆分](#62-表的拆分)
    - [6.3. 读写分离](#63-读写分离)
- [7. 系统调优](#7-系统调优)
    - [7.1. 查看 MySQL 服务器运行的状态值](#71-查看-mysql-服务器运行的状态值)
    - [7.2. 服务器参数调优](#72-服务器参数调优)
        - [7.2.1. 内存相关](#721-内存相关)
        - [7.2.2. IO 相关](#722-io-相关)
        - [7.2.3. 安全相关](#723-安全相关)
        - [7.2.4. 其他](#724-其他)
...
### explain extended
&emsp; 用explain extended查看执行计划会比explain多一列 filtered。filtered列给出了一个百分比的值，这个百分比值和rows列的值一起使用，可以估计出那些将要和explain中的前一个表进行连接的行的数目。前一个表就是指explain 的 id列的值比当前表的id小的表。  

## 4.2. show warnings

&emsp; SHOW WARNINGS显示有关由于当前会话中执行最新的非诊断性语句而导致的条件的信息。  
&emsp; mysql show warnings 避免一些隐似转换。  
