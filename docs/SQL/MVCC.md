...
- [1. MVCC](#1-mvcc)
    ...
    - [1.3. MVCC的实现](#13-mvcc的实现)
        - [1.4. 版本链的生成](#14-版本链的生成)
        - [1.5. Read View](#15-read-view)
        - [1.6. 不同隔离级别下的MVCC](#16-不同隔离级别下的mvcc)
            - [1.6.1. READ UNCOMMITTED](#161-read-uncommitted)
            - [1.6.2. READ COMMITTED](#162-read-committed)
            - [1.6.3. REPEATABLE READ，可重复读](#163-repeatable-read可重复读)
            - [1.6.4. SERIALIZABLE](#164-serializable)
            - [1.6.5. 总结](#165-总结)
            - [1.6.6. MVCC 解决了幻读了没有？](#166-mvcc-解决了幻读了没有)
...
* DB_ROLL_PTR：回滚指针，指向 undo log 记录。每次对某条记录进行改动时，该列会存一个指针，可以通过这个指针找到该记录修改前的信息 。<font color = "lime">当某条记录被多次修改时，该行记录会存在多个版本，通过回滚指针DB_ROLL_PTR 链接形成一个类似版本链的概念。</font>    
...
----
&emsp; 有了ReadView，这样在访问某条记录时，只需要按照下边的步骤判断记录的某个版本是否可见：  
...
&emsp; 在进行判断时，首先会拿记录的最新版本来比较，**<font color = "lime">如果该版本无法被当前事务看到，则通过记录的 DB_ROLL_PTR 找到上一个版本，重新进行比较，直到找到一个能被当前事务看到的版本。</font>**

&emsp; 而对于删除，其实就是一种特殊的更新，InnoDB 用一个额外的标记位 delete_bit 标识是否删除。当我们在进行判断时，会检查下 delete_bit 是否被标记，如果是，则跳过该版本，通过 DB_ROLL_PTR 拿到下一个版本进行判断。
...
谈到幻读，首先要引入“当前读”和“快照读”的概念，聪明的你一定通过名字猜出来了：    
...
其实原理也很简单，例如触发当前读：select * from user where id < 10 for update，当使用了 Gap Lock 时，Gap 锁会锁住 id < 10 的整个范围，因此其他事务无法插入 id < 10 的数据，从而防止了幻读。    
