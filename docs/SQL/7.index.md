...
- [1. Sql索引优化](#1-sql索引优化)
    ...
    - [1.5. 索引失效](#15-索引失效)
    - [1.6. 创建索引的几种方式](#16-创建索引的几种方式)
    - [1.7. 百万级别或以上的数据如何删除](#17-百万级别或以上的数据如何删除)
    - [1.8. 索引维护、索引工具的使用](#18-索引维护索引工具的使用)
...
## 1.7. 百万级别或以上的数据如何删除  
&emsp; 关于索引：由于索引需要额外的维护成本，因为索引文件是单独存在的文件,所以当我们对数据的增加,修改,删除,都会产生额外的对索引文件的操作,这些操作需要消耗额外的IO，会降低增/改/删的执行效率。  
&emsp; 所以，在删除数据库百万级别数据的时候，查询MySQL官方手册得知删除数据的速度和创建的索引数量是成正比的。  
