...
- [2. 慢查询，（监控）](#2-慢查询监控)
    ...
    - [2.2. 慢查询使用：](#22-慢查询使用)
    - [2.3. 慢查询工具：](#23-慢查询工具)
- [3. EXPLAIN、PROCEDURE ANALYSE，(分析)](#3-explainprocedure-analyse分析)
    - [3.1. EXPLAIN，分析查询](#31-explain分析查询)
    - [3.2. PROCEDURE ANALYSE()取得建议](#32-procedure-analyse取得建议)
- [4. Sql语句优化](#4-sql语句优化)
    - [4.1. 基本查询优化](#41-基本查询优化)
    - [4.2. 子查询优化](#42-子查询优化)
    - [4.3. 关联查询优化](#43-关联查询优化)
    - [4.4. 组合查询优化](#44-组合查询优化)
- [5. Sql索引优化](#5-sql索引优化)
    - [5.1. 索引简介](#51-索引简介)
    - [5.2. 创建索引原则](#52-创建索引原则)
    - [5.3. mysql建了多个单索引，查询多个条件时如何走索引的？](#53-mysql建了多个单索引查询多个条件时如何走索引的)
    - [5.4. 索引失效](#54-索引失效)
    - [5.5. 索引底层原理：](#55-索引底层原理)
        - [5.5.1. BTree索引：](#551-btree索引)
            - [5.5.1.1. B+ Tree简介：](#5511-b-tree简介)
            - [5.5.1.2. InnoDB引擎的底层实现：](#5512-innodb引擎的底层实现)
            - [5.5.1.3. MyISAM引擎的底层实现：](#5513-myisam引擎的底层实现)
            - [5.5.1.4. 为什么使用B+树？](#5514-为什么使用b树)
            - [5.5.1.5. 为什么官方建议使用自增长主键作为索引？](#5515-为什么官方建议使用自增长主键作为索引)
        - [5.5.2. Hash索引](#552-hash索引)
    - [5.6. 索引条件下推（ICP）](#56-索引条件下推icp)
    - [5.7. 索引维护、索引工具的使用](#57-索引维护索引工具的使用)

<!-- /TOC -->
# 1. MySql性能，最大数据量、最大并发数、查询耗时：  
* 最大数据量：  
&emsp; MySQL没有限制单表最大记录数，它取决于操作系统对文件大小的限制。  
![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-28.png)  
...
----
# 3. EXPLAIN、PROCEDURE ANALYSE，(分析)  
## 3.1. EXPLAIN，分析查询  

```sql
EXPLAIN SELECT column_name FROM table_name;  
```  
![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-29.png)  
...
* id：SELECT识别符。这是SELECT的查询序列号。  
    * id值相同  
    ...
    ![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-30.png)  
    &emsp; 两个id 都为1，先访问b表然后访问a表。  
    * id值不同  
    &emsp; id 值不同的情况，从大到小执行，值越大越先开始执行或者被访问。  
    ![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-31.png)  
    ...
    * id包含了相同和不同的情况。  
    ...
    ```
    ![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-32.png)  
...
## 5.3. mysql建了多个单索引，查询多个条件时如何走索引的？  
......
...
![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-35.png)  
...
![](https://gitee.com/wt1814/pic-host/raw/master/images/SQL/sql-39.png)  
&emsp; 哈希索引适用的场景：等值查询。  
