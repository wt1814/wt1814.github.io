...
- [1. Map](#1-map)
    - [1.1. HashMap](#11-hashmap)
        - [1.1.1. HashMap源码](#111-hashmap源码)
            - [1.1.1.1. HashMap类定义](#1111-hashmap类定义)
            - [1.1.1.2. HashMap数据结构](#1112-hashmap数据结构)
            - [1.1.1.3. 属性](#1113-属性)
            - [1.1.1.4. 构造函数](#1114-构造函数)
            - [1.1.1.5. 成员方法](#1115-成员方法)
                - [1.1.1.5.1. 通过K获取数组下标](#11151-通过k获取数组下标)
                - [1.1.1.5.2. put()，插入](#11152-put插入)
                - [1.1.1.5.3. resize()，扩容机制](#11153-resize扩容机制)
                - [1.1.1.5.4. remove()，删除](#11154-remove删除)
        - [1.1.2. HashMap在JDK1.7和JDK1.8中的区别总结](#112-hashmap在jdk17和jdk18中的区别总结)
        - [1.1.3. HashMap的线程安全问题？](#113-hashmap的线程安全问题)
        - [1.1.4. 如何实现一个自定义的class作为HashMap的key？](#114-如何实现一个自定义的class作为hashmap的key)
    - [1.2. LikedHashMap](#12-likedhashmap)
        - [1.2.1. ※※※LRU算法](#121-※※※lru算法)
...
&emsp; ***<font color = "red">为什么使用红黑树？</font>***  
...
* threshold数组扩容阈值。即：HashMap数组总容量 * 加载因子。***记录当前数组的最大容量。当前容量大于或等于该值时会执行扩容 resize()。***扩容的容量为当前HashMap总容量的两倍。比如，当前HashMap的总容量为16 ，那么扩容之后为32。  
...
### 1.1.2. HashMap在JDK1.7和JDK1.8中的区别总结  
