...
- [1. 集合类](#1-集合类)
    ...
    - [1.3. 集合使用：](#13-集合使用)
        - [1.3.1. Arrays.asList()不能使用其修改集合相关的方法](#131-arraysaslist不能使用其修改集合相关的方法)
        - [1.3.2. ArrayList修改元素的炕](#132-arraylist修改元素的炕)
        - [1.3.3. Java集合不能存放基本数据类型，只存放对象的引用](#133-java集合不能存放基本数据类型只存放对象的引用)
- [2. 迭代器类Iterator](#2-迭代器类iterator)
    ...
    - [2.4. ConcurrentModificationException异常](#24-concurrentmodificationexception异常)
    - [2.5. 快速失败（fail-fast）VS安全失败（fail-safe）](#25-快速失败fail-fastvs安全失败fail-safe)
- [3. 集合工具类](#3-集合工具类)
    - [3.1. 集合工具类Collections](#31-集合工具类collections)
    - [3.2. 数组工具类Arrays](#32-数组工具类arrays)
    - [3.3. Comparable、Comparator接口](#33-comparablecomparator接口)
...
## 2.4. ConcurrentModificationException异常  
...
&emsp; fail-fast 是 Java 中的一种快速失败机制，java.util 包下所有的集合都是快速失败的，快速失败会抛出 ConcurrentModificationException 异常，fail-fast 你可以把它理解为一种快速检测机制，它只能用来检测错误，不会对错误进行恢复，fail-fast 不一定只在多线程环境下存在，ArrayList 也会抛出这个异常，主要原因是由于 modCount 不等于 expectedModCount。    
...
&emsp; 同时存在时采用 Comparator（定制排序）的规则进行比较。  
