...
- [1. 哪些内存需要回收？](#1-哪些内存需要回收)
    - [1.1. GC 的存活标准](#11-gc-的存活标准)
...
- [2. 什么时候回收？生存还是死亡](#2-什么时候回收生存还是死亡)
- [方法区回收](#方法区回收)
- [3. 如何回收？（GC算法）](#3-如何回收gc算法)
    - [3.1. 标记-清除（Mark-Sweep）算法](#31-标记-清除mark-sweep算法)
    - [3.2. 复制（Copying）算法（新生代）](#32-复制copying算法新生代)
    - [3.3. 标记-整理（Mark-Compact）算法（老年代）](#33-标记-整理mark-compact算法老年代)
    - [3.4. 分代收集算法](#34-分代收集算法)
- [4. 垃圾回收器](#4-垃圾回收器)
    - [性能考虑因素](#性能考虑因素)
    - [收集器分类](#收集器分类)
    - [收集器详解](#收集器详解)
        - [4.1. --------新生代收集器](#41---------新生代收集器)
            - [4.2. Serial收集器，单线程收集器](#42-serial收集器单线程收集器)
            - [4.3. ParNew收集器，多线程收集器](#43-parnew收集器多线程收集器)
            - [4.4. Parallel Scavenge收集器，并行的多线程收集器](#44-parallel-scavenge收集器并行的多线程收集器)
        - [4.5. --------老年代收集器](#45---------老年代收集器)
            - [4.6. Serial Old收集器](#46-serial-old收集器)
            - [4.7. Parallel Old收集器](#47-parallel-old收集器)
            - [4.8. CMS收集器](#48-cms收集器)
        - [4.9. G1收集器](#49-g1收集器)
    - [4.10. --------](#410---------)
    - [4.11. 常用的收集器组合](#411-常用的收集器组合)
    - [4.12. 如何选择合适的垃圾收集器](#412-如何选择合适的垃圾收集器)
    - [4.13. 垃圾收集器常用参数：](#413-垃圾收集器常用参数)
- [5. GC类型：Minor GC和Full GC/Major GC](#5-gc类型minor-gc和full-gcmajor-gc)
...
&emsp; 虚拟机栈、本地栈和程序计数器在编译完毕后已经可以确定所需内存空间，程序执行完毕后也会自动释放所有内存空间，所以不需要进行动态回收优化。JVM GC回收堆区、方法区内的对象。  
![image](https://gitee.com/wt1814/pic-host/raw/master/images/java/JVM/JVM-49.png)  
...
&emsp; 设计思路：⽤⼀个HashMap来保存图⽚路径和相应图⽚对象关联的软引⽤之间的映射关系,在内 存不⾜时,JVM会⾃动共回收这些缓存图⽚对象所占的空间,避免OOM。  
...
&emsp; 即使在可达性分析算法中不可达的对象，也并非一定是“非死不可”的，这时候他们暂时处于“缓刑”阶段，真正宣告一个对象死亡至少要经历两个阶段：  
...
![image](https://gitee.com/wt1814/pic-host/raw/master/images/java/JVM/JVM-28.png)  
