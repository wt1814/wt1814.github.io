...
- [1. JVM类的加载](#1-jvm类的加载)
    ...
    - [1.3. 类加载的方式：类加载器](#13-类加载的方式类加载器)
        ...
        - [1.3.2. 类加载器的加载机制](#132-类加载器的加载机制)
            ...
            - [1.3.2.3. 自定义类加载器呢](#1323-自定义类加载器呢)
...
#### 1.2.2.2. 准备：  
&emsp; <font color = "red">准备阶段是正式为类变量分配内存并设置类变量初始值的阶段，这些内存都将在方法区中分配。</font>对于该阶段有以下几点需要注意：  
...
#### 1.2.2.3. 解析：  
&emsp; <font color = "red">解析阶段将常量池内的符号引用替换为直接引用的过程，得到类或者字段、方法在内存中的指针或者偏移量，确保类与类之间相互引用正确性，完成内存结构布局，以便直接调用该方法。</font>解析主要有四种：类或接口的解析、字段解析、类方法解析、接口方法解析。  

&emsp; ***解析过程在某些情况下可以在初始化阶段之后再开始，这是为了支持 Java 的动态绑定。***   

    直接引用：通过对符号引用进行解析，找到引用的实际内存地址。直接引用可以是指向目标的指针，相对偏移量或是一个能间接定位到目标的句柄。如果有了直接引用，那引用的目标必定已经在内存中存在。  
...
&emsp; 在准备阶段，类变量已经被赋过默认初始值，而在初始化阶段，类变量将被赋值为代码期望赋的值。初始化过程即执行类中的静态初始化代码（static代码块）、构造器代码以及静态变量的初始化。  



#### 1.2.3.1. 类初始化时机：  

1. ***<font color = "red">主动引用执行初始化。</font>***  
&emsp; <font color = "red">对于初始化阶段，虚拟机规范则是严格规定了有且只有5种情况必须立即对类进行“初始化”。</font>  
...
2. ***<font color = "red">被动引用不执行初始化。</font>***  
...
* 虚拟机会保证子client方法执行之前，父类的client方法已经执行完毕， 如果一个类中没有对静态变量赋值也没有静态语句块，那么编译器可以不为这个类生client()方法。  
    ...
    clinit方法和init方法：  
    在编译生成class文件时，会自动产生两个方法，clinit是类的初始化方法，init是实例的初始化方法。clinit：在jvm第一次加载class文件时调用，包括静态变量初始化语句和静态块的执行。init: 在实例创建出来的时候调用，包括调用new操作符；调用Class或 Java.lang.reflect.Constructor对象的newInstance()方法；调用任何现有对象的clone()方法；通过 java.io.ObjectInputStream 类的getObject()方法反序列化。  
