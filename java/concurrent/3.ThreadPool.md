...
- [2. 线程池框架Executor](#2-线程池框架executor)
    - [2.1. 线程池实现](#21-线程池实现)
        - [2.1.1. ThreadPoolExecutor-1](#211-threadpoolexecutor-1)
            - [2.1.1.1. 属性](#2111-属性)
            - [2.1.1.2. 构造函数](#2112-构造函数)
            - [2.1.1.3. ※※※线程池工作流程（execute成员方法的源码）](#2113-※※※线程池工作流程execute成员方法的源码)
        - [2.1.2. ForkJoinPool（JDK1.7新增）](#212-forkjoinpooljdk17新增)
            - [2.1.2.1. ForkJoinPool内部原理，工作窃取算法](#2121-forkjoinpool内部原理工作窃取算法)
                - [2.1.2.1.1. 分而治之](#21211-分而治之)
                - [2.1.2.1.2. 工作窃取(Work Stealing)算法](#21212-工作窃取work-stealing算法)
            - [2.1.2.2. API 描述](#2122-api-描述)
                - [2.1.2.2.1. 创建ForkJoinPool:](#21221-创建forkjoinpool)
                - [2.1.2.2.2. ForkJoinTask详解](#21222-forkjointask详解)
                - [2.1.2.2.3. 相关使用](#21223-相关使用)
        - [2.1.3. Executors的静态方法](#213-executors的静态方法)
...
- [3. 线程池正确用法](#3-线程池正确用法)
    ...
    - [3.5. SpringBoot整合线程池](#35-springboot整合线程池)
        - [3.5.1. 重写spring默认线程池](#351-重写spring默认线程池)
        - [3.5.2. 自定义线程池](#352-自定义线程池)
...
#### 2.1.2.1. ForkJoinPool内部原理，工作窃取算法
...
##### 2.1.2.1.2. 工作窃取(Work Stealing)算法  
...
## 3.1. 设置隔离的线程池
&emsp; 一些业务代码做了Utils类型在整个项目中的各种操作共享使用一个线程池，一些业务代码大量Java 8使用parallel stream特性做一些耗时操作但是没有使用自定义的线程池或是没有设置更大的线程数（没有意识到parallel stream的共享ForkJoinPool问题）。共享的问题在于会干扰，如果有一些异步操作的平均耗时是1秒，另外一些是100秒，这些操作放在一起共享一个线程池很可能会出现相互影响甚至饿死的问题。***<font color = "red">建议根据异步业务类型，合理设置隔离的线程池。</font>  
