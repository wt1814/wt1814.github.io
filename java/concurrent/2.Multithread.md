...
- [3. 线程同步](#3-线程同步)
    ...
    - [3.2. Synchronized](#32-synchronized)
        ...
        - [3.2.3. synchronized原理](#323-synchronized原理)
            ...
            - [synchronized与Object#wait()](#synchronized与objectwait)
        - [3.2.4. synchronized的锁优化](#324-synchronized的锁优化)
            - [synchronized锁升级](#synchronized锁升级)
                - [synchronized锁对比：](#synchronized锁对比)
            - [锁消除](#锁消除)
            - [锁粗化](#锁粗化)
    - [Volatile](#volatile)
        - [Volatile原理](#volatile原理)
        - [Volatile使用](#volatile使用)
            - [如何正确使用volatile变量](#如何正确使用volatile变量)
            - [示例一：状态标志](#示例一状态标志)
            - [单例模式的实现](#单例模式的实现)
    - [3.3. ThreadLocal](#33-threadlocal)
        - [ThreadLocal源码](#threadlocal源码)
            - [ThreadLocal存储结构](#threadlocal存储结构)
            - [ThreadLocal成员方法](#threadlocal成员方法)
                - [set()](#set)
                - [get()](#get)
        - [ThreadLocal使用](#threadlocal使用)
            - [使用示例：](#使用示例)
            - [SimpleDateFormat非线程安全问题](#simpledateformat非线程安全问题)
- [线程通信](#线程通信)
- [进程通信](#进程通信)
...
```
&emsp; 这个set操作和集合Map解决散列冲突的方法不同，集合Map采用的是链地址法，这里采用的是开放定址法（线性探测）。set()方法中的replaceStaleEntry()和cleanSomeSlots()，这两个方法可以清除掉key ==null的实例，防止内存泄漏。  
