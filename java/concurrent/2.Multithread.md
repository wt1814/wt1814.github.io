...
- [3. 线程同步](#3-线程同步)
    ...
    - [3.2. Synchronized](#32-synchronized)
        ...
        - [3.2.2. synchronized与ReentrantLock](#322-synchronized与reentrantlock)
        - [3.2.3. synchronized与Object#wait()](#323-synchronized与objectwait)
        - [3.2.4. synchronized原理](#324-synchronized原理)
            - [3.2.4.1. 反编译Synchronized代码块](#3241-反编译synchronized代码块)
                - [3.2.4.1.1. 同步代码块](#32411-同步代码块)
                - [3.2.4.1.2. 同步方法](#32412-同步方法)
        - [3.2.5. synchronized的锁优化](#325-synchronized的锁优化)
            - [3.2.4.1.2.1. Monitor和Java对象头详解](#324121-monitor和java对象头详解)
            - [自旋锁与自适应自旋](#自旋锁与自适应自旋)
            - [3.2.5.2. 锁消除](#3252-锁消除)
            - [3.2.5.3. 锁粗化](#3253-锁粗化)
            - [轻量级锁](#轻量级锁)
            - [偏向锁](#偏向锁)
            - [锁状态总结](#锁状态总结)
            - [3.2.5.1. synchronized锁升级](#3251-synchronized锁升级)
                - [3.2.5.1.1. synchronized锁对比](#32511-synchronized锁对比)
...
&emsp; ***总结：***  
&emsp; ***<font color = "red">出现线程安全问题的原因：线程切换带来的原子性问题；缓存不能及时刷新导致的可见性问题；编译器优化带来的有序性问题。“缓存不能及时刷新“和“编译器为了优化性能而改变程序中语句的先后顺序”都是重排序的一种。</font>***    
...
&emsp; ***<font color = "red">在并发编程需要处理的两个关键问题是：线程之间如何通信和线程之间如何同步，即两条标准的体现。</font>***  
&emsp; ***<font color = "red">线程通信是一种手段，而线程同步是一种目的，即线程通信的主要目的是用于线程同步。线程同步是为了解决线程安全问题。</font>***
...
&emsp; ***<font color= "red">内存屏障是被插入两个 CPU 指令之间的一种指令，用来禁止处理器指令发生重排序，从而保障有序性的。另外，为了达到屏障的效果，它也会使处理器写入、读取值之前，将主内存的值写入高速缓存，清空无效队列，从而保障可见性。</font>***  
&emsp; 例如：
...
### 3.2.2. synchronized与ReentrantLock  
...
### 3.2.3. synchronized与Object#wait()  
...
&emsp; 轻量锁就是先通过CAS操作进行同步，因为绝大部分的锁，在整个同步周期都是不存在线程去竞争的。  
...
2. <font color = "red">禁止进行指令重排序，实现了有序性。</font>也就是说，在volatile变量的赋值操作后⾯会有⼀个内存屏障（⽣成的汇编代码上），读操作不会被重排序到内存屏障之前。  
...
&emsp; ***<font color = "red">ynchronized和volatile比较：</font>***  
