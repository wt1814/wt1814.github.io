...
- [2. java.util.concurrent.locks包](#2-javautilconcurrentlocks包)
    - [2.1. ReentrantLock，重入锁](#21-reentrantlock重入锁)
        - [2.1.1. ReentrantLock的API](#211-reentrantlock的api)
            - [2.1.1.1. 构造函数](#2111-构造函数)
            - [2.1.1.2. 成员方法](#2112-成员方法)
        - [2.1.2. ReentrantLock与synchronized比较](#212-reentrantlock与synchronized比较)
        - [2.1.3. 使用示例](#213-使用示例)
    - [2.2. Condition，等待/通知机制](#22-condition等待通知机制)
        - [2.2.1. Condition 类提供的方法](#221-condition-类提供的方法)
        - [2.2.2. Condition 与 wait/notify](#222-condition-与-waitnotify)
        - [2.2.3. 使用示例](#223-使用示例)
    - [2.3. ReentrantReadWriteLock，读写锁](#23-reentrantreadwritelock读写锁)
        - [2.3.1. 示例](#231-示例)
    - [2.4. StampedLock，读写锁的升级](#24-stampedlock读写锁的升级)
        - [2.4.1. StampedLock的三种模式](#241-stampedlock的三种模式)
        - [2.4.2. 使用示例](#242-使用示例)
- [3. Atomic，原子类](#3-atomic原子类)
    ...
    - [3.3. AtomicStampedReference类详解](#33-atomicstampedreference类详解)
        - [3.3.1. 源码分析](#331-源码分析)
        - [3.3.2. 示例](#332-示例)
    ...
    - [4.3. Map，ConcurrentHashMap、ConcurrentSkipListMap](#43-mapconcurrenthashmapconcurrentskiplistmap)
        - [4.3.1. ConcurrentHashMap](#431-concurrenthashmap)
            - [4.3.1.1. Java7 ConcurrentHashMap](#4311-java7-concurrenthashmap)
                - [4.3.1.1.1. 存储结构](#43111-存储结构)
                    - [4.3.1.1.1.1. 静态内部类 Segment](#431111-静态内部类-segment)
                    - [4.3.1.1.1.2. 静态内部类HashEntry](#431112-静态内部类hashentry)
                - [4.3.1.1.2. 构造函数](#43112-构造函数)
                - [4.3.1.1.3. 成员方法](#43113-成员方法)
                - [4.3.1.1.4. put()方法](#43114-put方法)
                    - [4.3.1.1.4.1. 扩容 rehash](#431141-扩容-rehash)
                - [4.3.1.1.5. get()方法](#43115-get方法)
            - [4.3.1.2. Java8 ConcurrentHashMap](#4312-java8-concurrenthashmap)
                - [4.3.1.2.1. 存储结构](#43121-存储结构)
                - [4.3.1.2.2. 初始化 initTable](#43122-初始化-inittable)
                - [4.3.1.2.3. 成员方法](#43123-成员方法)
                    - [4.3.1.2.3.1. put()方法](#431231-put方法)
                    - [4.3.1.2.3.2. get()方法](#431232-get方法)
...
&emsp; ***乐观读锁如何保证数据一致性呢？***  
&emsp; 乐观读锁在获取 stamp 时，会将需要的数据拷贝一份出来。在真正进行读取操作时，验证 stamp 是否可用。如何验证 stamp 是否可用呢？从获取 stamp 到真正进行读取操作这段时间内，如果有线程获取了写锁，stamp 就失效了。如果 stamp 可用就可以直接读取原来拷贝出来的数据，如果 stamp 不可用，就重新拷贝一份出来用。我们操作的是方法栈里面的数据，也就是一个快照，所以最多返回的不是最新的数据，但是一致性还是得到保障的。  
...
&emsp; ***jdk1.8 中的 ConcurrentHashMap中synchronized只锁定当前链表或红黑树的首节点，只要节点hash不冲突，就不会产生并发，相比JDK1.7的 ConcurrentHashMap效率又提升了N倍！***  

##### 4.3.1.2.2. 初始化 initTable  
...
&emsp; java.util.concurrent.CountDownLatch类，能够使一个线程等待其他线程完成各自的工作后再执行。利用它可以实现类似计数器的功能。比如有一个任务A，它要等待其他4个任务执行完毕之后才能执行，此时就可以利用CountDownLatch来实现这种功能了。  
