...
- [1. SpringMVC注解](#1-springmvc注解)
    - [1.1. 地址映射](#11-地址映射)
        - [1.1.1. Content-Type介绍](#111-content-type介绍)
    - [1.2. 绑定参数](#12-绑定参数)
        - [1.2.1. 自定义转换](#121-自定义转换)
            - [1.2.1.1. 注解@DateTimeFormat、@JsonFormat](#1211-注解datetimeformatjsonformat)
            - [1.2.1.2. Converter，转换器](#1212-converter转换器)
            - [1.2.1.3. Formatter，格式化](#1213-formatter格式化)
    - [1.3. 返回模型和视图](#13-返回模型和视图)
- [2. SpringMVC高级功能](#2-springmvc高级功能)
    - [2.1. SpringMVC中文乱码](#21-springmvc中文乱码)
    - [2.2. SpringMVC全局异常处理器](#22-springmvc全局异常处理器)
    - [2.3. SpringMVC拦截器](#23-springmvc拦截器)
    - [2.4. SpringMVC上传图片](#24-springmvc上传图片)
    - [2.5. SpringMVC解决跨域问题](#25-springmvc解决跨域问题)
...
&emsp; 数据转换方式：1.硬编码（javaBean中日期类型为String）；2.SpringMVC自定义转换(javaBean中日期类型为Date)。方式二需要自定义转换器（Converter）或格式化（Formatter）进行数据绑定。  

#### 1.2.1.1. 注解@DateTimeFormat、@JsonFormat  
...
## 2.2. SpringMVC全局异常处理器  
&emsp; 参考[异常处理](../../java/basis/JavaException.md)  

## 2.3. SpringMVC拦截器  
&emsp; 参考[过滤器、拦截器、监听器](../../web/subassembly.md)  
