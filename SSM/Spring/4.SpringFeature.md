...
```
&emsp; ~~BeanPostProcessor处理器，可以在bean初始化前、后根据自己业务做一些事情。postProcessBeforeInitialization方法里面，里面invokeAwareInterfaces方法是怎样都会走的，里面有这样一段代码((ApplicationContextAware) bean).setApplicationContext(this.applicationContext)；这就是去调用具体实现类的setApplicationContext方法把applicationContext传进去了。~~  
