...
- [2. Spring Core](#2-spring-core)
    ...
    - [2.3. SpringBean的作用域](#23-springbean的作用域)
    - [2.3.1. Spring中的单例bean的线程安全问题：](#231-spring中的单例bean的线程安全问题)
...
## 2.1. Spring IOC  
&emsp; Spring配置方式有三种：基于XML的配置、基于注解的配置、基于Java的配置。  

## 2.2. Spring DI  

&emsp; Spring的IOC有三种注入方式 ：构造器注入、setter方法注入、根据注解注入。  

&emsp; ***<font color = "red">自动装配：</font> ***  
&emsp; 在spring中，对象无需自己查找或创建与其关联的其他对象，由容器负责把需要相互协作的对象引用赋予各个对象，使用autowire来配置自动装载模式。  
