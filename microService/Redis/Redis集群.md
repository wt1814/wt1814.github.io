...
- [主从复制模式：](#主从复制模式)
    ...
    - [主从复制应用与问题](#主从复制应用与问题)
        ...
        - [规避全量复制](#规避全量复制)
            - [第一次复制](#第一次复制)
            - [节点运行 ID 不匹配](#节点运行-id-不匹配)
            - [复制偏移量 offset 不在复制积压缓冲区中](#复制偏移量-offset-不在复制积压缓冲区中)
        - [规避复制风暴](#规避复制风暴)
            - [单一主节点](#单一主节点)
            - [单一机器](#单一机器)
- [哨兵模式](#哨兵模式)
...
&emsp; 在主从复制的架构模式下，我们知道，所有的写操作都发生在主节点，删除数据时也是在主节点删除，然后将删除命令同步给从节点，由于主从节点的延迟性，主节点删除了某些数据，从节点不一定也删除了，所以在从节点是比较容易读到过期的数据。但是在 Redis 3.2 版本中，增加了对数据是否过期的判断，如果读到的数据过期了，则不会返回给客户端了，所以将 Redis 升级到 3.2 版本后即可解决该问题。  
