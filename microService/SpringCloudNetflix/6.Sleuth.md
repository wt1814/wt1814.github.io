...
- [全链路监控功能模块](#全链路监控功能模块)
- [Sleuth实现追踪](#sleuth实现追踪)
- [跟踪原理](#跟踪原理)
- [spring-cloud-starter-sleuth功能点](#spring-cloud-starter-sleuth功能点)
    - [抽样收集](#抽样收集)
    - [获取当前traceId](#获取当前traceid)
    - [日志获取traceId](#日志获取traceid)
    - [传递traceId到异步线程池](#传递traceid到异步线程池)
    - [子线程或线程池中获取 Zipkin traceId 并打印](#子线程或线程池中获取-zipkin-traceid-并打印)
    - [监控本地方法](#监控本地方法)
    - [计划任务](#计划任务)
    - [TracingFilter](#tracingfilter)
    - [过滤不想跟踪的请求](#过滤不想跟踪的请求)
    - [用RabbitMq代替Http发送调用链数据](#用rabbitmq代替http发送调用链数据)
- [与Zipkin整合](#与zipkin整合)
    - [在Zipkin中图形化展示分布式链接监控数据](#在zipkin中图形化展示分布式链接监控数据)
    - [链路信息收集](#链路信息收集)
        - [消息中间件收集](#消息中间件收集)
    - [数据持久化](#数据持久化)
    - [API接口](#api接口)
...
&emsp; 在SpringBoot应用中中引入spring-cloud-starter-sleuth依赖之后，<font color = "red">sleuth会自动为当前应用构建起各通信通道的跟踪机制</font>，比如： 
