...
- [4. spring-cloud-starter-sleuth功能点](#4-spring-cloud-starter-sleuth功能点)
    - [4.1. 抽样收集](#41-抽样收集)
        - [4.1.1. 获取当前traceId](#411-获取当前traceid)
    - [4.2. 日志获取traceId](#42-日志获取traceid)
    - [4.3. 传递traceId到异步线程池](#43-传递traceid到异步线程池)
    - [4.4. 子线程或线程池中获取 Zipkin traceId 并打印](#44-子线程或线程池中获取-zipkin-traceid-并打印)
    - [4.5. 监控本地方法](#45-监控本地方法)
    - [4.6. 计划任务](#46-计划任务)
    - [4.7. TracingFilter](#47-tracingfilter)
    - [4.8. 过滤不想跟踪的请求](#48-过滤不想跟踪的请求)
    - [4.9. 用RabbitMq代替Http发送调用链数据](#49-用rabbitmq代替http发送调用链数据)
- [5. 与Zipkin整合](#5-与zipkin整合)
    - [5.1. 链路信息收集：](#51-链路信息收集)
        - [5.1.1. HTTP收集：](#511-http收集)
        - [5.1.2. 消息中间件收集](#512-消息中间件收集)
    - [5.2. 在Zipkin中图形化展示分布式链接监控数据：](#52-在zipkin中图形化展示分布式链接监控数据)
    - [5.3. 数据持久化：](#53-数据持久化)
        - [5.3.1. 将链路数据存储在Mysql数据库](#531-将链路数据存储在mysql数据库)
        - [5.3.2. 将链路数据存储在ElasticSearch](#532-将链路数据存储在elasticsearch)
...
## 4.4. 子线程或线程池中获取 Zipkin traceId 并打印  
......
