...
- [1. 路由转发](#1-路由转发)
    - [1.1. 服务路由的默认规则](#11-服务路由的默认规则)
    - [1.2. 路径匹配](#12-路径匹配)
    - [1.3. zuul的安全与header](#13-zuul的安全与header)
    - [1.4. 重定向问题](#14-重定向问题)
    - [1.5. 使用zuul上传文件问题](#15-使用zuul上传文件问题)
    - [1.6. Hystrix 和 Ribbon 支持](#16-hystrix-和-ribbon-支持)
- [2. 请求过滤](#2-请求过滤)
    - [2.1. 前置过滤器](#21-前置过滤器)
        - [2.1.1. 鉴权](#211-鉴权)
        - [2.1.2. 流量转发](#212-流量转发)
...
- [3. 动态加载](#3-动态加载)
- [4. zuul的高可用](#4-zuul的高可用)
    - [4.1. Zuul客户端也注册到了Eureka Server上](#41-zuul客户端也注册到了eureka-server上)
    - [4.2. Zuul客户端未注册到Eureka Server上](#42-zuul客户端未注册到eureka-server上)
...
## 4.1. Zuul客户端也注册到了Eureka Server上  
