...
- [1. 在Spring Cloud中基于Eureka的Spring Boot Admin的搭建](#1-在spring-cloud中基于eureka的spring-boot-admin的搭建)
- [2. Spring boot Admin依赖](#2-spring-boot-admin依赖)
    - [2.1. actuator原生端点](#21-actuator原生端点)
        - [2.1.1. 度量指标类](#211-度量指标类)
- [3. 监控通知](#3-监控通知)
- [4. 集成spring security](#4-集成spring-security)

<!-- /TOC -->

![image](https://gitee.com/wt1814/pic-host/raw/master/images/microService/SpringCloudNetflix/cloud-34.png)  

# 1. 在Spring Cloud中基于Eureka的Spring Boot Admin的搭建  
......

# 2. Spring boot Admin依赖
&emsp; Spring Boot Admin依赖了spring-boot-starter-actuator，引入该依赖能够自动为 Spring Boot 构建的应用提供 一系列用于监控的端点。在Spring Boot工程中新增spring-boot-starter-actuator依赖。具体如下：  
...
&emsp; 有时候，可能还会用到一些Spring Boot的Starter POMs中还没有封装的产品来进行开发，比如：当使用RocketMQ作为消息代理时，由于没有自动化配置的检测器，所以需要自己来实现一个用来采集健康信息的检测器。比如，可以在Spring Boot的应用中，为org.springframework.boot.actuate.health.HealthIndicator接口实现一个对RocketMQ的检测器类：  
    ...
    ```  
* ***<font color = "red">/heapdump</font>***：访问http://localhost:8080/actuator/heapdump 会自动生成一个 Jvm 的堆文件 heapdump。可以使用 JDK 自带的 Jvm 监控工具 VisualVM 打开此文件查看内存快照。  

* ***<font color = "red">/dump</font>***：该端点用来暴露程序运行中的线程信息。它使用java.lang.management.ThreadMXBean的dumpAllThreads方法来返回所有含有同步信息的活动线程详情。  
